...
class CCobraEvaluator():
    ...
    def __init__(self, modellist, eval_comparator, test_datafile, train_datafile=None,
                 ...
                 domain_encoders=None, cache_df=None, params = {}):
        """
        ...
        """
        self.datafilename = test_datafile
        self.params = params
        ...
        for id_info, subj_df in ccobra_data.get().groupby('id'):
            ...
            for _, row in subj_df.sort_values(['sequence']).iterrows():
                ...
                train_dict['item'] = ccobra.Item(
                    ...
                    row['choices'], row['sequence'])#, [row['wm1'],row['wm2'],row['wm3'],row['wm4']])
                ...
                #if counter % 10000 == 0:
                    #print('Train-Data Lines', counter)
    ...
    def evaluate(self):
        ...
        for idx, modelinfo in enumerate(self.modellist):
            ...
            with contextmanager.dir_context(modelinfo.path):
                ...
                print('minA')
                print(modelinfo.args)
                pre_model = importer.instantiate(modelinfo.args, self.params)
                ...
                for subj_id, subj_df in self.test_data.get().groupby('id'):
                    ...
                    for _, row in subj_df.sort_values('sequence').iterrows():
                        ...
                        domain = row['domain']
                        wm = [] #[row['wm1'],row['wm2'],row['wm3'],row['wm4']]
                        ...
                        item = ccobra.data.Item(
                            subj_id, domain, task, response_type, choices, sequence, wm)
...
class AdaptionEvaluator(CCobraEvaluator):
    ...
    def evaluate(self):
        ...
        """
        print('evalunating, please wait a very long time ' + self.datafilename)
        result_data = []
        model_name_cache = set() if self.cache_df is None else set(self.cache_df['model'].unique())
        ...
        for idx, modelinfo in enumerate(self.modellist):
            # Print the progress
            if not self.silent:
                print("Evaluating '{}' ({}/{})...".format(modelinfo.path, idx + 1, len(self.modellist)))
            ...
            with contextmanager.dir_context(modelinfo.path):
                ...
                pre_model = importer.instantiate(model_kwargs = self.params)
                ...
                for subj_id, subj_df in self.test_data.get().groupby('id'):
                    ...
                    for _, row in subj_df.sort_values('sequence').iterrows():
                        ...
                        domain = row['domain']
                        isTesting = False
                        #print('answering', row, 'phase' in row.keys() and row['phase'] != "-",'phaseType' in row.keys() or 'PhaseType' in row.keys(), int(row['phase']) <= 2)    
                        if 'phase' in row.keys() and row['phase'] != "-":
                            if 'phaseType' in row.keys() or 'PhaseType' in row.keys():
                                isTesting = False if int(row['phase']) <= 2 else True
                            else:    
                                isTesting = False if int(row['phase']) % 2 == 1 else True
                        else:
                            if 'trial' in row.keys() and int(row['trial']) < 0:
                                isTesting = False
                            else:
                                isTesting = True
                        if isinstance(truth, str):
                            if response_type == 'multiple-choice':
                                ...
                                truth = [x.split(';') for x in truth.split('/')]
                        if 'isTesting' in row.keys():
                            #print(row['isTesting'], type(row['isTesting']))
                            isTesting = bool(row['isTesting'])
                        modeTT = 'testing' if isTesting else 'training'
                        item = ccobra.data.Item(
                            subj_id, domain, task, response_type, choices, sequence, isTesting)
                        model = model.execute(subj_id)
                        prediction = model.predict(copy.deepcopy(item), **optionals)
                        hit = int(self.comparator.compare(prediction, truth))
                        taskCopy = copy.deepcopy(item).choices
                        pair = (taskCopy[0][0][0], taskCopy[1][0][0])
                        pair = (str(min(int(a) for a in pair)), str(max(int(a) for a in pair)))
                        pred = model.predictS(model.sortedPair(pair))
                        pred_perc = min(1, max(0,pred)) if int(min(pair)) == int(row['response']) else 1 - min(1, max(0,pred))
                        # Adapt to true response
                        adapt_item = ccobra.data.Item(
                            subj_id, domain, task, response_type, choices,
                            sequence, isTesting)
                        if not isTesting or model.adaptInTesting:
                        ...
                        prediction_data.update({
                            'modeTT': modeTT,
                            'percOfCorrPred' : pred_perc,
                            #'parameters' : str(self.params),
                            'idPhase' : row['phase'] if 'phase' in row else 'nophase',
                            'task_enc': task_enc,
                            'truth_enc': truth_enc,
                            'prediction_enc': pred_enc
                        })
