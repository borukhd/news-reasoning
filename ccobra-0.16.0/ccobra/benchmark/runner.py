...
def parse_arguments():
    ...
    parser.add_argument('-p', '--param', type=str, help='Parameter list.')
...
def main(args):
    ...
    if isinstance(args, str):
        line = args.replace('  ',' ').split(' ')
        args = {}
        for wordInd in range(len(line)):
            if '-s' in line[wordInd]:
                args['save']= line[wordInd + 1]
            if '-o' in line[wordInd]:
                args['output']= line[wordInd + 1]
            if '-p' in line[wordInd]:
                args['param'] = line[wordInd + 1].replace('\'','')
            args['benchmark'] = line[-1].replace('~','~')
            args['cache'] = False
            args['model'] = False

    modellist = []
    if args['model']:
        modellist.append(bmark.ModelInfo(args['model'], os.getcwd(), args['classname'], args['param']))

    # Load the benchmark settings
    benchmark = None
    modelCommandsDict = args['param']
    modelCommandsDict = modelCommandsDict.replace('X',' ').replace('[','[\'').replace(':', '\':').replace(', ', '\', \'').replace('{','{\'').replace(']','\']').replace('\']\'','\']')
    exec('modelCommandsDict = ' +  modelCommandsDict)
    benchmark = bmark.load_benchmark(args['benchmark'], modelCommandsDict)
    ...
    print('Done Reading Benchmark')

    print('n-start, ', False)
    print('start, ', is_silent)
    res_df = eva.evaluate()
    if 'save' in args:
