...
class RecommenderPlinear(ccobra.CCobraModel):
    ...
    def __init__(self, name='RecommenderPersonLinear', commands = []):
        ...
        #self.relevant = ['education', 'accimp','crt']#, 'panasPos', 'panasNeg']
    ...
    def similar(self, person1, person2):
        difference = sum(abs((RS.featuresOfAllPeople[person1][key] - RS.featuresOfAllPeople[person2][key])*self.parameter[key]) for key in self.relevant)
        if difference < 1:
            return True
        else: 
            return False
